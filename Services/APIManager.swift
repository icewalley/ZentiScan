...
class APIManager: ObservableObject {
    ...
    func generateChecklist(for code: String, context: String? = nil, location: String? = nil)
        async throws -> GenerateChecklistResponse
    {
    ...
    func searchNS3457(query: String) async throws -> [NS3457CodeInfo] {
        guard
            ...
            let url = URL(string: "\(baseURL)/ns3457/search?q=\(encodedQuery)")
        else {
            throw URLError(.badURL)
        }
